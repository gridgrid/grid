(function () {
    describe('add dirty props root', function () {
        ...
        var makeDirtyClean = require('../dirty-clean').create;
        require('../grid-spec-helper')();

        var ctx = {
            props: ['random', 'props', 'are', 'fun']
        };
        beforeEach(function () {
            var grid = this.buildSimpleGrid();
            var parentDirtyClean = makeDirtyClean(grid);
            var parent = {
                isDirty: parentDirtyClean.isDirty
            };
            ...
            ctx.props.push(prop);
            ctx.obj = require('../dirty-props').add({}, ctx.props, [dirtyClean, parentDirtyClean]);
